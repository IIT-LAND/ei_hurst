function result = C_1b_parcellate(atlasfile, datafile, fname2save, MEANCENTER, nreg)
%   C_1b_parcellate - Extract mean time series from parcellation
%
...
for i = 1:length(parc_num)
    ...
    if sum(mask(:))==0
        ...
        for ivol = 1:size(data,4)
            ...
            result(ivol,i) = mean(tmp_vol(mask));
        end % for i
    end % if
end % for ivol
...
end % function C_1b_parcellate(atlasfile, datafile, fname2save, MEANCENTER, nreg)
